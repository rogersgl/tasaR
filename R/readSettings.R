...
readSettings <- function(x, row = NULL){
  ...
  if (length(x) == 1 && stringr::str_detect(x, ".csv")) {
    ...
    out <- t[(row-1),]
  } else if (length(x) > 1 || (is(x, "data.frame") && ncol(x) > 1)) {
    message("Processing 'x' as an R object.\n")
    if (is(x, "list") && length(x) != length(snames)) {stop("Invalid length for list 'x'.")}
    if (is(x, "data.frame") && ncol(x) != length(snames)) {stop("Invalid column number for data.frame 'x'.")}
    out <- as.data.frame(x)
  } else {
...
#' @examples
#' \dontrun{
#'   buildSettings(Name = "test",
#'     IsAntibody = TRUE,
#'     MergedFASTQPath = file.path(tempdir(), "test-merged.fastq"),
#'     ReferenceSequence = "GTTCAACTGGTGGAAAGCGG...",
...
buildSettings <- function(Name = 'unknown',
  ...
  new("tas.object.settings", Name = Name,
      ...
      ReferenceSequence = list(ReferenceSequence),
...
defineSettingsObject <- function(df, snames) {
  if (!is(df, "data.frame")) {stop("Invalid input class. Must be a data.frame.")}
