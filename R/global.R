...
#' @param config Determines where the function draws the analysis parameters from. 'manual' allows paramaters to be specified in the function.
...
#' @import ggplot2
#' @import ggseqlogo
#' @rawNamespace import(seqinr, except = translate)
#' @rawNamespace import(R.utils, except = countLines)
#' @rawNamespace import(ape, except = c(zoom, consensus, as.alignment, complement))
#' @rawNamespace import(ShortRead, except = tables)
#' @rawNamespace import(Biostrings, except = c(pairwiseAlignment, pattern, nindel, errorSubstitutionMatrices, alignedPattern, alignedSubject, stringDist, pid, nedit, mismatchSummary, writePairwiseAlignments, mismatchTable, nucleotideSubstitutionMatrix, PairwiseAlignments, qualitySubstitutionMatrices, compareStrings, deletion, unaligned, indel, insertion, PairwiseAlignmentsSingleSubject, aligned))
#' @import pwalign
#' @import msa
#' @import ggmsa
...
tas_analyze <- function(InputFilePath,
  ...
  config <- match.arg(config,c('manual'))
  ...
  #                         PhyloTree = TRUE,
  #                         dna.repair.pathways = TRUE)
  # NGS$Config <- c(NGS$Config, manual.settings)
  ...
  tas_check(InputFilePath,NGS$Input, NGS$Config, WD)
  ...
  if (NGS$Config$merge.reads==1){
    if (NGS$Config$OperatingSystem %in% c("MacOS","Linux")){
      ...
      tas_pandaseq_terminal(Sample.Names, NGS$Input, NGS$Config, WD)
